...
[点击立即下载](https://github.com/CongShuoNiu/gossh/blob/master/bin)
...
Usage of gossh:
  ...
  -timeout int
        ...
        单台主机 SSH 命令执行、文件推送、文件拉取的超时时间，单位为秒，默认值是10。
  ...
  -knownhosts string
        ...
        SSH host key 校验文件路径，默认使用 ~/.ssh/known_hosts。

  -insecure-ignore-host-key
        skip SSH host key verification
        跳过 SSH host key 校验。仅建议在可信网络或临时应急恢复场景使用。
...
要实现对远程机器的免密批量操作，每台目标机器只需完成以下两个步骤：

#### 步骤一：信任 SSH host key

gossh 默认通过 `known_hosts` 校验 SSH host key，防止中间人攻击。需要先将目标机器的 key 加入信任列表：

```bash
ssh-keyscan <目标IP> >> ~/.ssh/known_hosts

# 批量添加多台机器：
ssh-keyscan -f ip列表.txt >> ~/.ssh/known_hosts
```

验证是否添加成功：

```bash
ssh-keygen -F <目标IP> -f ~/.ssh/known_hosts
```

#### 步骤二：选择认证方式

**方式 A — GSSAPI/Kerberos（企业环境推荐）**

如果你的公司使用 Kerberos 认证（如 AD 域或 FreeIPA），只需执行一次 `kinit` 获取票据，gossh 会自动使用票据完成认证，无需输入密码：
...
# gossh 直接连接，无需 -p 参数
gossh -t cmd -h <目标IP> -u <用户名> "hostname"
```

> **注意**：GSSAPI 功能需要开启 CGO 编译（`CGO_ENABLED=1 go build`）。macOS 使用系统 GSS 框架；Linux 需安装 `libgssapi-krb5-2` 或等效包。

**方式 B — 密码认证（明文或密文）**

直接传递密码，或在批量 IP 文件中配置密码：

```bash
# 明文密码
gossh -t cmd -h <目标IP> -u root -p <密码> "hostname"

# 密文密码（先用 passtool 加密）
gossh -t cmd -h <目标IP> -u root -p <密文> -e -key <密钥> "hostname"
```

#### 验证

完成上述两步后，免密批量操作即可正常工作：

```bash
# GSSAPI 方式（无需 -p）
gossh -t cmd -h <目标IP> -u <用户名> "hostname && whoami"
...
- gossh 默认通过 `~/.ssh/known_hosts` 校验 SSH host key，可以通过 `-knownhosts` 指定其他文件。如果目标机器尚未写入 known_hosts，可以先使用 `ssh-keyscan` 生成；`-insecure-ignore-host-key` 会跳过校验，仅建议在可信网络或临时应急恢复场景使用。
- `-timeout` 是单台主机维度的超时时间，适用于远程命令执行、push 和 pull。传输大文件时建议显式设置更大的超时时间，避免文件传输过程中被默认 10 秒超时中断。
- 批量操作会在逐台输出后追加 summary，统计总数、成功数、失败数、跳过数和失败目标主机。只要任意目标主机失败或超时，gossh 进程退出码为非 0。
- SCP push 和 pull 成功后会输出传输观测字段：`transfer_bytes`、`transfer_duration`、`transfer_throughput`。

### 4.5 大文件批量传输

从 A 机器向 B/C/D 等多台机器批量推送大文件时，建议根据文件大小和网络带宽调大 `-timeout`。该超时时间会分别作用于每台目标机器。

```
gossh -t push -i ip.txt -c 3 -timeout 3600 /path/big.tar.gz /data/
```

如果开启默认 host key 校验，请确保目标机器已经写入 `~/.ssh/known_hosts`，也可以显式指定 known_hosts 文件：

```
gossh -t push -i ip.txt -c 3 -timeout 3600 -knownhosts ~/.ssh/known_hosts /path/big.tar.gz /data/
```

文件传输成功后，每台目标机器会输出传输指标：
...
点击[示例](https://github.com/CongShuoNiu/gossh/blob/master/docs/example.md)查看详情。

### 4.7 输出和日志

点击[输出和日志](https://github.com/CongShuoNiu/gossh/blob/master/docs/output_format.md)查看详情。


## 5.密码管理

点击[密码管理](https://github.com/CongShuoNiu/gossh/blob/master/docs/password.md)查看详情。

## 6.安全性

gossh从多种角度保证执行安全，包括密码的加密存放、命令黑名单、以及文件传递过程中的检查、日志记录等，详情
点击[gossh安全管理](https://github.com/CongShuoNiu/gossh/blob/master/docs/safe.md)查看详情。

gossh 默认通过 `known_hosts` 校验 SSH host key，降低中间人攻击风险。默认路径为 `~/.ssh/known_hosts`；如果该文件不存在，可以通过 `ssh-keyscan` 生成，或通过 `-knownhosts path` 指定其他文件。`-insecure-ignore-host-key` 会保留旧版本跳过校验的行为，仅建议在可信网络或临时应急恢复场景使用。
...
![qq群](https://github.com/CongShuoNiu/gossh/raw/master/docs/images/gossh_qq.png)
